// ---------------------------------------------------------------------------
// ctsv_format_registration.hpp -- opt-in synthesis of stream-insertion
// operators for types that are formattable but not stream-insertable.
// ---------------------------------------------------------------------------
//
// This header was extracted verbatim from the former ctsv_assoc_container.hpp,
// whose name had become actively misleading: it contained no associative
// container at all. Its two unrelated halves now live where they belong --
// the comparison function objects moved to ctsv_comparators.hpp, and the
// formatting / stream-registration machinery is here.
//
// WHAT IT DOES
//
// Given a type T that has a std::formatter (or a fmt::formatter) but no
// operator<<, you can obtain stream insertion for free by specializing one of
// the g_k_*_stream_insert_via_*_format variable templates to true. The
// constrained operator<< templates below then pick T up via ADL.
//
// Usage directions and a worked example: see fmt_reg_demo.hpp / .cpp in
// test_console_app.
//
// RULES (enforced by the concepts, not merely documented)
//
//   * Do not register a type that is ALREADY stream insertable. Doing so
//     produces either an ambiguity or a silent switch between the declared and
//     the synthesized operator.
//   * Do not register the same type for BOTH std::format and fmt::format. The
//     concepts below reject any type registered for both.
//   * If your type is both std_formattable and fmt_formattable, simply pick
//     one and register for that one only.
//   * The narrow and wide facilities are independent of one another.
// ---------------------------------------------------------------------------
#ifndef CPS_CT_STR_CTSV_FORMAT_REGISTRATION_HPP
#define CPS_CT_STR_CTSV_FORMAT_REGISTRATION_HPP
...
namespace cps::ct_string
{
    ...
    /// \brief A type that std::format can render to a narrow string.
    template<typename T>
    ...
    /// \brief A type that std::format can render to a wide string.
    template<typename T>
...
#   endif
    ...
    /// \brief A type that fmt::format can render to a narrow string.
    template<typename T>
...
#    endif
    ...
    /// \brief A type that fmt::format can render to a wide string.
    template<typename T>
...
#   endif
    ...
    /// \brief A type that can already be inserted into a narrow stream.
    /// Registering such a type for format-synthesis is an error.
    template<typename T>
    ...
    /// \brief A type that can already be inserted into a wide stream.
    template<typename T>
    ...
    // Specialize exactly one of these to true for your type. See the header
    // comment for the rules.

    /// \brief Register a std_formattable type for narrow stream insertion.
    template<std_formattable T>
    inline constexpr bool g_k_stream_insert_via_std_format = false;

    /// \brief Register a fmt_formattable type for narrow stream insertion.
    template<fmt_formattable T>
    inline constexpr bool g_k_stream_insert_via_fmt_format = false;

    /// \brief Register a std_wide_formattable type for wide stream insertion.
    template<std_wide_formattable T>
    inline constexpr bool g_k_wide_stream_insert_via_std_format = false;

    /// \brief Register a fmt_wide_formattable type for wide stream insertion.
    ...
    /// \brief Registered for narrow insertion via std::format (and not also
    /// via fmt::format -- registering for both is rejected here).
    template<typename T>
    ...
    /// \brief Registered for narrow insertion via fmt::format (and not also
    /// via std::format).
    template<typename T>
    ...
    /// \brief Registered for narrow insertion via either formatting library.
    template<typename T>
    ...
    /// \brief Registered for wide insertion via std::format.
    template<typename T>
    ...
    /// \brief Registered for wide insertion via fmt::format.
    template<typename T>
    ...
    /// \brief Registered for wide insertion via either formatting library.
    template<typename T>
    concept wide_stream_insertable_via_format =
        wide_stream_insertable_via_std_format<T> || wide_stream_insertable_via_fmt_format<T>;

    /// \brief Registered for insertion into a stream of either width.
    template<typename T>
    ...
    /// \brief Synthesized narrow stream insertion for registered types.
    /// \remarks Found by ADL on cps::ct_string. Because it is constrained on
    /// the registration variable template, it does not participate for any
    /// type that has not explicitly opted in.
    ...
    /// \brief Synthesized wide stream insertion for registered types.
...
#endif // CPS_CT_STR_CTSV_FORMAT_REGISTRATION_HPP
